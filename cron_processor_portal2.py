...
# -*- coding: utf-8 -*-
"""
CRON-процессор для второго портала Битрикс24
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('/var/log/bitrix_cron_portal2.log'),
...
class BitrixAPI:
    ...
    def __init__(self, webhook_url):
        ...
        self.session.headers.update({
            ...
            'User-Agent': 'BitrixCronProcessorPortal2/1.0'
        })
    ...
    def get_contact_deals(self, contact_id, limit=50):
        ...
        return self._make_request('crm.deal.list', {
            ...
            'select': ['ID', 'TITLE', 'STAGE_ID', 'UF_CRM_1758612743092'],
...
class DealProcessor:
    ...
    def __init__(self, api_client):
        self.api = api_client
        self.rejection_reason_field = 'UF_CRM_1758612743092'  # Предыдущие причины отказов
        self.rejection_history_field = 'UF_CRM_1758612825886'  # Причина отказа
        self.max_field_length = 2000
        self.rejection_stages = ['LOSE']  # Стадии отказов
    ...
    def process_deal(self, deal_id):
        ...
        try:
            logger.info(f"Processing deal {deal_id} on portal 2")
...
def main():
    """Основная функция"""
    try:
        logger.info("=== CRON PROCESSOR PORTAL 2 STARTED ===")
        
        # Инициализация API для второго портала
        webhook_url = "https://b24-j9fqpa.bitrix24.ru/rest/9/cu1f85dy254v7guk"
        ...
        logger.info(f"Found {len(recent_deals)} recent deals on portal 2")
        ...
        logger.info(f"=== CRON PROCESSOR PORTAL 2 COMPLETED: {processed_count} deals processed ===")
        
    except Exception as e:
        logger.error(f"CRON processor portal 2 error: {e}")
